...
const Catalogo = () => {
    ...
    const categorias = useMemo(() => ['Todas', ...new Set(catalogoLibros.map(libro => libro.category))], []);
    const autores = useMemo(() => ['Todos', ...new Set(catalogoLibros.map(libro => libro.author))], []);

    const librosFiltrados = catalogoLibros.filter(libro => {
        const porBusqueda = libro.title.toLowerCase().includes(searchTerm.toLowerCase()) || libro.author.toLowerCase().includes(searchTerm.toLowerCase());
